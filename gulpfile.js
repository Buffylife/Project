...
const buildVideo    = "src/build/video";
...
gulp.task("video", function() {
  return gulp.src("src/blocks/**/*.{mp4,webm}")
    // Выгрузка
    .pipe(gulpif(dist,
      combine(
        gulp.dest(distVideo)
...
gulp.task("watch", function(c) {
  if (!dist) { // Проверяет на наличие флага.
    ...
    gulp.watch("src/blocks/**/*.{mp4,webm}", gulp.series("video"));
...
gulp.task("build",
  gulp.series(
    "clean", "css", "libs", "jsCustom", "html", "imgs","video", "imgsSvg", "fonts", "svg", "cleanManifest"
  )
);
